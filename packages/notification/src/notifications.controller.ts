...
const permissionGuard = new PermissionGuard(
  ...
  new PermissionCheckClient({
    ...
    serviceApiKey: resolveOwnServiceApiKey('notification'),
  }),
);
...
export class NotificationController {
  ...
  async send(@Body() body: { recipientUserId: string; message: string }, @CurrentAuth() auth: AccessTokenClaims) {
    await auditEmitter.emit({
      tenantId: auth.tenantId, actorUserId: auth.sub, service: 'notification',
